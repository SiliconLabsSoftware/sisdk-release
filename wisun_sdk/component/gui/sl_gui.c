...
__STATIC_INLINE void _gui_mutex_acquire(void)
{
  EFM_ASSERT(osMutexAcquire(_gui_mtx, osWaitForever) == osOK);
}

/* Mutex release */
__STATIC_INLINE void _gui_mutex_release(void)
{
  EFM_ASSERT(osMutexRelease(_gui_mtx) == osOK);
}
#endif
