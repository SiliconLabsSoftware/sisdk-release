...
void sl_zigbee_af_end_device_support_status_command(sl_cli_command_arg_t *arguments)
{
  ...
  sl_zigbee_af_cli_println("End Device Poll Information");
  sl_zigbee_af_cli_println("EMBER_END_DEVICE_TIMEOUT:       %d", SL_ZIGBEE_END_DEVICE_POLL_TIMEOUT);
  sl_zigbee_af_cli_println("Poll completed callback: %s",
  ...
  for (i = 0; i < SL_ZIGBEE_SUPPORTED_NETWORKS; i++) {
    ...
    if (sli_zigbee_af_pro_is_current_network()
      ...
      sl_zigbee_af_cli_println("  Current Poll Interval (qs):   %ld",
                               sl_zigbee_af_get_current_poll_interval_qs_cb());
      sl_zigbee_af_cli_println("  Long Poll Interval (qs):      %ld",
      ...
      if (SL_ZIGBEE_SLEEPY_END_DEVICE <= sli_zigbee_af_current_zigbee_pro_network->nodeType) {
        sl_zigbee_af_cli_println("  Short Poll Interval (qs):     %ld",
                                 sl_zigbee_af_get_short_poll_interval_qs_cb());
        sl_zigbee_af_cli_flush();
        sl_zigbee_af_cli_println("  Wake Timeout (qs):            %ld",
                                 sl_zigbee_af_get_wake_timeout_qs_cb());
        sl_zigbee_af_cli_flush();
        sl_zigbee_af_cli_println("  Wake Timeout Bitmask:         0x%08X",
                                 sl_zigbee_af_get_wake_timeout_bitmask_cb());
        sl_zigbee_af_cli_flush();
        sl_zigbee_af_cli_println("  Current App Tasks:            0x%08X",
                                 sl_zigbee_af_get_current_app_tasks_cb());
        sl_zigbee_af_cli_println("  Current Poll Control          %s",
                                 pollControlStrings[sl_zigbee_af_get_current_poll_control_cb()]);
        sl_zigbee_af_cli_flush();
        sl_zigbee_af_cli_println("  Default Poll Control          %s",
...
void sl_zigbee_af_end_device_support_force_short_poll_command(sl_cli_command_arg_t *arguments)
{
  ...
  if (shortPoll) {
    sl_zigbee_af_cli_println("Forcing device to short poll");
    sl_zigbee_af_add_to_current_app_tasks_cb(SL_ZIGBEE_AF_FORCE_SHORT_POLL);
  } else {
    sl_zigbee_af_cli_println("Allowing device to long poll");
