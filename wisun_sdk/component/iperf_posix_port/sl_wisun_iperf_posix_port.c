...
int32_t sl_iperf_socket_accept(int32_t sockid, sl_iperf_socket_addr_t *addr)
{
  ...
  return accept(sockid, (struct sockaddr *)addr, &addrlen);
}
                                 ...
                                 sl_iperf_socket_addr_t *src_addr)
{
  socklen_t addrlen  = sizeof(sockaddr_in6_t);
  EFM_ASSERT(buf != NULL);
  return recvfrom(sockid, buf, len, 0U, (struct sockaddr *) src_addr, &addrlen);
}
...
sl_iperf_ts_ms_t sl_iperf_get_timestamp_ms(void)
{
  ...
  return (sl_iperf_ts_ms_t) sl_sleeptimer_tick_to_ms(tick_cnt);
}
