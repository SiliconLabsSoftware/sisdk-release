/***************************************************************************//**
 * @file sl_zigbee_mac_app_events_ipc_callback_events.c
 * @brief callback event handlers for sl_zigbee_mac_app_events
 *******************************************************************************
 * # License
 * <b>Copyright 2026 Silicon Laboratories Inc. www.silabs.com</b>
 *******************************************************************************
 *
 * The licensor of this software is Silicon Laboratories Inc. Your use of this
 * software is governed by the terms of Silicon Labs Master Software License
 * Agreement (MSLA) available at
 * www.silabs.com/about-us/legal/master-software-license-agreement. This
 * software is distributed to you in Source Code format and is governed by the
 * sections of the MSLA applicable to Source Code.
 *
 ******************************************************************************/
// automatically generated from sl_zigbee_mac_app_events.h.  Do not manually edit
#include "stack/internal/src/ipc/sl_zigbee_mac_app_events_ipc_callback_events.h"
#include "stack/internal/src/ipc/zigbee_ipc_callback_events.h"
#include "stack/include/multi-network.h"
extern void sl_zigbee_wakeup_common_task(void);
extern sl_status_t sl_zigbee_af_push_network_index(uint8_t networkIndex);
extern sl_status_t sl_zigbee_af_pop_network_index(void);

void sli_802154_stackmac_communication_status_indication_handler(uint8_t mac_index,
                                                                 sl_status_t status,
                                                                 uint8_t packet_length,
                                                                 uint8_t *packet_data)
{
  sl_zigbee_stack_cb_event_t *cb_event = (sl_zigbee_stack_cb_event_t *) malloc(sizeof(sl_zigbee_stack_cb_event_t));
  if (cb_event == NULL) {
    assert(false); // "ipc callback event allocation failed
    return;
  }
  cb_event->data.sl_802154mac_communication_status_indication_handler.mac_index = mac_index;
  cb_event->data.sl_802154mac_communication_status_indication_handler.status = status;
  cb_event->data.sl_802154mac_communication_status_indication_handler.packet_length = packet_length;

  if (packet_length > MAX_IPC_VEC_ARG_CAPACITY) {
    assert(false); // "vector packet_data length exceeds expected maximum
    packet_length = MAX_IPC_VEC_ARG_CAPACITY;
  }

  if (packet_data != NULL) {
    memmove(cb_event->data.sl_802154mac_communication_status_indication_handler.packet_data, packet_data, sizeof(uint8_t) * packet_length);
  }

  cb_event->tag = SLI_802154_STACKMAC_COMMUNICATION_STATUS_INDICATION_HANDLER_IPC_EVENT_TYPE;
  #ifndef SL_ZIGBEE_MULTI_NETWORK_STRIPPED
  cb_event->network_idx = sl_zigbee_get_callback_network();
  #endif // !SL_ZIGBEE_MULTI_NETWORK_STRIPPED
  sl_event_publish(&sli_zigbee_ipc_publisher, SL_EVENT_CLASS_ZIGBEE, 1 /*priority*/, cb_event);
  sl_zigbee_wakeup_common_task();
}

void sli_zigbee_sl_zigbee_mac_app_events_process_ipc_event(sl_zigbee_stack_cb_event_t *cb_event)
{
  #ifndef SL_ZIGBEE_MULTI_NETWORK_STRIPPED
  (void)sl_zigbee_af_push_network_index(cb_event->network_idx);
  #endif // !SL_ZIGBEE_MULTI_NETWORK_STRIPPED
  switch (cb_event->tag) {
    case SLI_802154_STACKMAC_COMMUNICATION_STATUS_INDICATION_HANDLER_IPC_EVENT_TYPE:
      sl_802154mac_communication_status_indication_handler(cb_event->data.sl_802154mac_communication_status_indication_handler.mac_index,
                                                           cb_event->data.sl_802154mac_communication_status_indication_handler.status,
                                                           cb_event->data.sl_802154mac_communication_status_indication_handler.packet_length,
                                                           cb_event->data.sl_802154mac_communication_status_indication_handler.packet_data);
      break;

    default:
      /* do nothing */
      break;
  }
  #ifndef SL_ZIGBEE_MULTI_NETWORK_STRIPPED
  sl_zigbee_af_pop_network_index();
  #endif // !SL_ZIGBEE_MULTI_NETWORK_STRIPPED
}
