...
/* SL custom patch: for test code, turn off irrelevant IAR warnings leading to
 * failed builds. By turning off the error codes here instead of globally, we
 * can compile the test code as provided, while still getting notified if any
 * of the core code throws IAR warnings. This header is only included from
 * test code. */
#if defined(__IAR_SYSTEMS_ICC__)
// Pe068: "integer conversion resulted in a change of sign"
// Thrown when the compiler thinks you made an unintended mistake against integer
// promotion rules. Can be turned off for test code due to its autogenerated
// nature, but should be fixed in core code.
#pragma diag_suppress=Pe068
// Pe111: "Statement is unreachable"
// Usually triggered due to a switch case with conditional-compiled case entries.
// Should the configuration be set in such a way that all case entries are
// compiled out, and only the default case remains, and that default case returns,
// then IAR will mark the remainder of the function as dead code. To be fixed in
// the core code, but for test functions we can just turn off the warning.
#pragma diag_suppress=Pe111
// Pe188: "Enumerated type mixed with another type"
// Thrown in the autogenerated test code because the test vector inputs are
// integer values, meaning any enum input part of a vector will be converted
// to integer and cast back to an enum. This is acceptable for test code, but
// not for core code.
#pragma diag_suppress=Pe188
// Pe236: "Controlling expression is constant". Due to how the test suite is
// dynamically generated from test data input, there are some condition
// statements where the compiler can see that the expression evaluates to a
// constant, which it isn't happy about.
#pragma diag_suppress=Pe236
// Pe546: "Transfer of control bypasses initialization"
// Occurs extremely often in PSA Core code, since its coding paradigm is very
// centered around 'goto exit' when a function call fails. Basically, IAR throws
// this warning if there are variables which are declared/initialised between
// a goto statement and the label it would go to.
// Resolution for this error is to put the code using those inline-declared
// variables into an explicit scope block, which tells the compiler the
// inline-declared variables are only meant for use in that scope, and hopping
// over the full scope won't trigger the warning.
// To be fixed with custom patches to the core, and hopefully one day upstreamed.
