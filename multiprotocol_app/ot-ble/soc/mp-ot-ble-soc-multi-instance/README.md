...
To build the mp-ot-ble-soc-multi-instance sample app from source you must have installed SSv5 and the SiSDK. The GNU ARM toolchain is installed with SSv5. The IAR-EWARM toolchain is not compatible with OpenThread.
...
2. The Example Project Selection dialog opens. Use the Technology Type and Keyword filters to search for a specific example, in this case mp-ot-ble-soc-multi-instance. Select it and click NEXT.
...
The Simplicity IDE opens with the mp-ot-ble-soc-multi-instance project open in the Project Configurator. You may now build the project. The mp-ot-ble-soc-multi-instance.s37 image will be located in the **GNU ARM <version>** directory, and may be uploaded to your board using an SSv5 tool such as the flash programmer or Simplicity Commander.
...
The mp-ot-ble-soc-multi-instance app adds multi-instance support with the following commands:
...
The mp-ot-ble-soc-multi-instance app also includes all the Bluetooth commands from the original BLE DMP application. Type "ble" at the prompt to see a list of subcommands:
