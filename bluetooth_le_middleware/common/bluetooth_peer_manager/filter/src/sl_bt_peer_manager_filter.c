...
#include "sl_bt_peer_manager_filter_config.h"

#if defined(SL_CATALOG_APP_LOG_PRESENT) && defined(SL_BT_PEER_MANAGER_FILTER_LOG)
  #include "app_log.h"
#endif // defined(SL_CATALOG_APP_LOG_PRESENT) && defined(SL_BT_PEER_MANAGER_FILTER_LOG)
...
#if defined(SL_CATALOG_APP_LOG_PRESENT) && SL_BT_PEER_MANAGER_FILTER_LOG
#define sl_bt_peer_manager_filter_log_debug(...)           app_log_debug(__VA_ARGS__)
#define sl_bt_peer_manager_filter_log_info(...)            app_log_info(__VA_ARGS__)
#define sl_bt_peer_manager_filter_log_error(...)           app_log_error(__VA_ARGS__)
#define sl_bt_peer_manager_filter_hexdump(p_data, len)     app_log_hexdump_debug(p_data, len)
#else
#define sl_bt_peer_manager_filter_log_debug(...)
#define sl_bt_peer_manager_filter_log_info(...)
#define sl_bt_peer_manager_filter_log_error(...)
#define sl_bt_peer_manager_filter_hexdump(p_data, len)
#endif
...
typedef struct sl_bt_peer_manager_filter_s {
...
} sl_bt_peer_manager_filter_t;

// -----------------------------------------------------------------------------
// Static variables
static sl_bt_peer_manager_filter_t active_filter;

// -----------------------------------------------------------------------------
// Public functions
void sl_bt_peer_manager_filter_init(void)
{
...
void sl_bt_peer_manager_set_filter_bt_address(bool enabled)
{
  if (enabled) {
    active_filter.filter_set.flags.address = FILTER_SET;
    sl_bt_peer_manager_filter_log_info("BT address filtering enabled" APP_LOG_NL);
  } else {
    active_filter.filter_set.flags.address = FILTER_CLEARED;
    sl_bt_peer_manager_filter_log_info("BT address filtering disabled" APP_LOG_NL);
  }
}

sl_status_t sl_bt_peer_manager_add_allowed_bt_address(bd_addr *filter_addr)
{
  if (!sl_bt_peer_manager_is_filter_set_allowed()) {
  ...
  for (uint8_t i = 0u; i < MAX_NUMBER_OF_ADDRESS_FILTERS; i++) {
    if (memcmp(&(active_filter.address[i].addr), empty_addr.addr, sizeof(bd_addr)) == 0) {
      ...
      sl_bt_peer_manager_filter_log_info("BT address accept filter added for '%02x:%02x:%02x:%02x:%02x:%02x'" APP_LOG_NL,
  ...
  sl_bt_peer_manager_filter_log_error("Could not add BT address accept filter, maximum address filter number (%u) reached"
                                      APP_LOG_NL,
                                      MAX_NUMBER_OF_ADDRESS_FILTERS);
  return SL_STATUS_NO_MORE_RESOURCE;
}

sl_status_t sl_bt_peer_manager_remove_allowed_bt_address(bd_addr *filter_addr)
{
  if (filter_addr == NULL) {
    return SL_STATUS_INVALID_PARAMETER;
  }
  ...
  for (uint8_t i = 0u; i < MAX_NUMBER_OF_ADDRESS_FILTERS; i++) {
    if (memcmp(&(active_filter.address[i].addr), filter_addr->addr, sizeof(bd_addr)) == 0) {
      ...
      sl_bt_peer_manager_filter_log_info("BT address accept filter cleared for '%02x:%02x:%02x:%02x:%02x:%02x'" APP_LOG_NL,
                                         active_filter.address[i].addr[5],
                                         active_filter.address[i].addr[4],
                                         active_filter.address[i].addr[3],
                                         active_filter.address[i].addr[2],
...
sl_status_t sl_bt_peer_manager_set_filter_address_type(sl_bt_gap_address_type_t addr_type)
{
  if (!sl_bt_peer_manager_is_filter_set_allowed()) {
    return SL_STATUS_INVALID_STATE;
  }
  ...
  sl_bt_peer_manager_filter_log_info("Filtering by address type, only accepting '%02x'" APP_LOG_NL,
                                     active_filter.address_type);
  return SL_STATUS_OK;
}

sl_status_t sl_bt_peer_manager_set_filter_device_name(const char *device_name,
                                                      uint8_t device_name_len,
                                                      bool full_match)
{
  if (!sl_bt_peer_manager_is_filter_set_allowed()) {
  ...
  sl_bt_peer_manager_filter_log_info("Filtering by device name, only accepting '%s'" APP_LOG_NL,
                                     active_filter.device_name);
  return SL_STATUS_OK;
}

sl_status_t sl_bt_peer_manager_set_filter_service_uuid16(sl_bt_uuid_16_t *service_uuid16)
{
  if (!sl_bt_peer_manager_is_filter_set_allowed()) {
    return SL_STATUS_INVALID_STATE;
  }
  ...
  sl_bt_peer_manager_filter_log_info("Filtering by 16 bit UUID" APP_LOG_NL);
  return SL_STATUS_OK;
}

sl_status_t sl_bt_peer_manager_set_filter_service_uuid128(uuid_128 *service_uuid128)
{
  if (!sl_bt_peer_manager_is_filter_set_allowed()) {
    return SL_STATUS_INVALID_STATE;
  }
  ...
  sl_bt_peer_manager_filter_log_info("Filtering by 128 bit UUID" APP_LOG_NL);
  return SL_STATUS_OK;
}

sl_status_t sl_bt_peer_manager_set_filter_service_data(uint8_t *service_data,
                                                       uint8_t service_data_offset,
                                                       uint8_t service_data_len)
{
  if (!sl_bt_peer_manager_is_filter_set_allowed()) {
  ...
  sl_bt_peer_manager_filter_log_info("Filtering by service data" APP_LOG_NL);
  return SL_STATUS_OK;
}

sl_status_t sl_bt_peer_manager_set_filter_manufacturer_data(uint8_t *manufacturer_data,
                                                            uint8_t manufacturer_data_offset,
                                                            uint8_t manufacturer_data_len)
{
  if (!sl_bt_peer_manager_is_filter_set_allowed()) {
  ...
  sl_bt_peer_manager_filter_log_info("Filtering by manufacturer data" APP_LOG_NL);
  return SL_STATUS_OK;
}

sl_status_t sl_bt_peer_manager_set_filter_rssi(int8_t rssi)
{
  if (!sl_bt_peer_manager_is_filter_set_allowed()) {
    return SL_STATUS_INVALID_STATE;
  }
  ...
  sl_bt_peer_manager_filter_log_info("Filtering by RSSI, only accepting from %d dBm" APP_LOG_NL,
                                     active_filter.rssi);
  return SL_STATUS_OK;
}

bool sl_bt_peer_manager_is_filter_set(void)
{
...
sl_status_t sl_bt_peer_manager_reset_filter(void)
{
  if (!sl_bt_peer_manager_is_filter_set_allowed()) {
    return SL_STATUS_INVALID_STATE;
  }
...
bool sl_bt_peer_manager_find_match(bd_addr *address,
                                   sl_bt_gap_address_type_t address_type,
                                   int8_t rssi,
                                   const uint8array *adv_data)
{
  ...
  if (active_filter.filter_set.flags.address == FILTER_SET) {
    ...
    for (uint8_t i = 0u; i < MAX_NUMBER_OF_ADDRESS_FILTERS; i++) {
      if (memcmp(&(active_filter.address[i].addr), address->addr, sizeof(bd_addr)) == 0) {
        sl_bt_peer_manager_filter_log_debug("Found matching BT address" APP_LOG_NL);
        found = true;
        break;
      }
    }
  ...
  if (active_filter.filter_set.flags.address_type == FILTER_SET) {
    ...
    sl_bt_peer_manager_filter_log_debug("Found matching address type" APP_LOG_NL);
  }
  if (active_filter.filter_set.flags.rssi == FILTER_SET) {
    ...
    sl_bt_peer_manager_filter_log_debug("Found matching RSSI" APP_LOG_NL);
  }
      ...
      if ((advertisement_type == AD_TYPE_SHORTENED_LOCAL_NAME) || (advertisement_type == AD_TYPE_COMPLETE_LOCAL_NAME)) {
        ...
        if (advertisement_length - 1 >= active_filter.device_name_len
          ...
          if (memcmp(&adv_data->data[i + 2], active_filter.device_name, active_filter.device_name_len) == 0) {
            sl_bt_peer_manager_filter_log_debug("Found matching device name" APP_LOG_NL);
            found_name = true;
          }
        }
      }
      ...
      if ((advertisement_type == AD_TYPE_INCOMPLETE_LIST_16_BIT_SERVICE_IDS)
        ...
        for (uint8_t j = 0; j < (advertisement_length - 1); j = j + sizeof(sl_bt_uuid_16_t)) {
          if (memcmp(&(active_filter.service_uuid16),
            ...
            sl_bt_peer_manager_filter_log_debug("Found matching 16 bit UUID" APP_LOG_NL);
      ...
      if ((advertisement_type == AD_TYPE_INCOMPLETE_LIST_128_BIT_SERVICE_IDS)
        ...
        for (uint8_t j = 0; j < (advertisement_length - 1); j = j + sizeof(uuid_128)) {
          if (memcmp(&(active_filter.service_uuid128),
            ...
            sl_bt_peer_manager_filter_log_debug("Found matching 128 bit UUID" APP_LOG_NL);
    ...
    if (active_filter.filter_set.flags.service_data == FILTER_SET) {
      if (advertisement_type == AD_TYPE_SERVICE_DATA) {
        ...
        if (memcmp(&(active_filter.service_data),
                   ...
                   active_filter.service_data_len) == 0) {
          sl_bt_peer_manager_filter_log_debug("Found matching service data" APP_LOG_NL);
        } else {
          return false;
        }
      }
    ...
    if (active_filter.filter_set.flags.manufacturer_data == FILTER_SET) {
      if (advertisement_type == AD_TYPE_MANUFACTURER_DATA) {
        ...
        if (memcmp(&(active_filter.manufacturer_data),
                   ...
                   active_filter.manufacturer_data_len) == 0) {
          sl_bt_peer_manager_filter_log_debug("Found matching manufacturer data" APP_LOG_NL);
        } else {
          return false;
        }
      }
  ...
  sl_bt_peer_manager_filter_log_info("Found matching device based on the configured filters" APP_LOG_NL);
  return true;
}

SL_WEAK bool sl_bt_peer_manager_is_filter_set_allowed(void)
{
  return true;
}

sl_status_t sl_bt_peer_manager_str_to_address(const char *str, bd_addr *address)
{
