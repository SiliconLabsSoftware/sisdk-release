...
__STATIC_INLINE void _rht_mutex_acquire(void)
{
  EFM_ASSERT(osMutexAcquire(_rht_sensor_mtx, osWaitForever) == osOK);
}

/* Mutex release */
__STATIC_INLINE void _rht_mutex_release(void)
{
  EFM_ASSERT(osMutexRelease(_rht_sensor_mtx) == osOK);
}
#endif
...
void sl_wisun_rht_init(void)
{
  ...
  EFM_ASSERT(_rht_sensor_mtx != NULL);
