# Internal Storage Bootloader (single slot, 2 MB device)

Demonstrates how to configure the Gecko Bootloader to store firmware update images using internal main flash, in a single-slot setup with a 988 kB slot starting at 0x100000.

This project provides a **storage-based** Gecko Bootloader for Silicon Labs Series 2 SoCs that have **2048 kB (2 MB) of internal main flash**. The bootloader does not itself receive the upgrade image over a communication link; instead the running application downloads the GBL upgrade image (over any transport — BLE, Wi-SUN, OpenThread, Zigbee, UART, etc.) and writes it into a dedicated storage slot in internal flash. On the next reboot the bootloader verifies the image in the slot and installs it over the application. A single storage slot is configured, sized `1007616` bytes (`0xF6000`) starting at `0x100000` (or `0x8100000` on parts whose flash is mapped at `0x08000000`). The slot layout can be adjusted on the **Storage** tab of the project configurator.
...
This specific sample is configured for Silicon Labs Series 2 SoCs with **2 MB of internal flash**. It enables exactly one storage slot (`SLOT0_ENABLE=1`) sized `1007616` bytes (`0xF6000`), starting at `0x100000` on parts with a zero-based flash map or `0x8100000` (= 135266304) on parts whose flash is mapped at `0x08000000`. The correct base address is selected automatically by `device_sdid_*` conditions in the `.slcp`, so the same project configuration builds for any 2 MB Series 2 device of those families (sdid 200, 205, 210, 215, 220, 225). The `bootloader_common_storage_single` component enforces a single-slot configuration, `bootloader_aes_sha_ecdsa` provides secure-boot and authenticated-upgrade support, and `bootloader_token_management` preserves manufacturing tokens across upgrades.

## Prerequisites / Setup Requirements

**Hardware**
- A Silicon Labs Series 2 SoC development kit with a part that has **2048 kB (2 MB) of internal flash** and is supported by the Gecko Platform SDK. The `hardware:device:flash:2048` tag and `device_sdid_200/205/210/215/220/225` conditions restrict this sample to those families (typically EFR32xG21 / xG22 / xG23 / xG24 / xG27 / xG28 2 MB radio boards on a Wireless Pro Kit or Wireless Starter Kit mainboard).
...
1. In Simplicity Studio, create a new project from **Bootloader - SoC Internal Storage (single image on 2MB device)** for your target Series 2 / 2 MB SoC.
...
- **Wrong-size part:** This sample assumes **2048 kB (2 MB)** of internal flash and is restricted to `device_sdid_200/205/210/215/220/225` parts. On any other family the bootloader will fail to build or will overlap the application. Switch to the matching size variant (`...-352k`, `...-512k`, `...-768k`, `...-1016k`, `...-single` for 1024 kB, `...-1536k`, `...-1920k`, `...-3200k`).
...
- **Slot write fails at a specific offset:** Verify the slot base and size against the chip's flash page boundaries — writes must be aligned and the full slot must fit inside main flash without overlapping the bootloader or application regions. The slot must end at or before `0x1F6000` (`0x100000 + 0xF6000`), or `0x081F6000` on parts mapped at `0x08000000`.
- **Upgrade image does not fit:** The slot is `0xF6000` bytes (~984 kB) by default. If your application image plus GBL metadata exceeds that size, either enable compression (see the `-lzma` storage variant) or reduce the application footprint; on a 2 MB part you can grow the slot somewhat by trimming the application region in the linker.
