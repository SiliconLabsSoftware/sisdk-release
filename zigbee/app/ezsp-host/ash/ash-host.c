...
sl_zigbee_ezsp_status_t ashResetNcp(void)
{
  ...
  switch (sli_ash_host_get_config(resetMethod)) {
    ...
    default:
      ...
      status = SL_ZIGBEE_EZSP_HOST_FATAL_ERROR;
      break;
  } // end of switch(sli_ash_host_get_config(resetMethod)
  return status;
}
...
{
  sl_zigbee_ezsp_status_t status;
  if (sli_ash_host_get_config(resetMethod) != ASH_RESET_METHOD_NONE) {
    sl_zigbee_ezsp_serial_read_flush();
  }
  ashSetAndStartAckTimer(sli_ash_host_get_config(timeRst));
...
static sl_zigbee_ezsp_status_t ashReceiveFrame(void)
{
  ...
  if (!(ashFlags & FLG_CONNECTED)) {
    ...
    if (frameType == TYPE_RSTACK) {
      ...
      ashStopAckTimer();
      ashTimeouts = 0;
      ashSetAckPeriod(sli_ash_host_get_config(ackTimeInit));
      ...
      case SEND_STATE_IDLE:
        ...
        if (ashFlags & FLG_RST) {
          ...
          // Send a DATA frame if ready
                    ...
                    && WITHIN_RANGE(ackRx, frmTx, ackRx + sli_ash_host_get_config(txK) - 1) ) {
...
static void ashDataFrameFlowControl(void)
{
  ...
  if (ashFlags & FLG_CONNECTED) {
    ...
    if (freeRxBuffers < sli_ash_host_get_config(nrLowLimit)) {
      ashFlags |= FLG_NR;
    } else if (freeRxBuffers > sli_ash_host_get_config(nrHighLimit)) {
      ashFlags &= ~FLG_NR;
...
static void ashRandomizeBuffer(uint8_t *buffer, uint8_t len)
{
  if (sli_ash_host_get_config(randomize)) {
