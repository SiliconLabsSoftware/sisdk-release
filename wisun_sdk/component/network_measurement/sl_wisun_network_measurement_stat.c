                               ...
                               const bool print)
{
  ...
  EFM_ASSERT(sl_wisun_get_neighbor_count(&neighbors_count) == SL_STATUS_OK);

  EFM_ASSERT(neighbors_count <= (SL_WISUN_MAX_NODE_COUNT - 1));

  EFM_ASSERT(sl_wisun_get_neighbors(&neighbors_count, _mac_addr_storage) == SL_STATUS_OK);
...
__STATIC_INLINE void _nwm_stat_mutex_acquire(void)
{
  EFM_ASSERT(osMutexAcquire(_nwm_stat_mtx, osWaitForever) == osOK);
}

/* Mutex release */
__STATIC_INLINE void _nwm_stat_mutex_release(void)
{
  EFM_ASSERT(osMutexRelease(_nwm_stat_mtx) == osOK);
}
