# CLI Kernel Micrium OS

This example demonstrates how to use the CLI driver with a Micrium OS kernel task over the VCOM serial port.
...
This example shows the command line interface (CLI) component running in a Micrium OS kernel task. Use the CLI over the kit's VCOM serial port. Sample commands: echo_str (echo arguments as strings), echo_int (parse and echo integers), LED (turn on, off, or toggle board LEDs). Use it to add CLI to a Micrium OS-based application.
...
- **No response to commands:** Confirm the terminal is on the correct port and baud rate; check Micrium OS task and CLI initialization.
