                                                         ...
                                                         bool bootloaderMode)
{
...
#ifdef EZSP_CPC
  // CPC mode doesn't use traditional serial port setup, return fatal error
  // since serial port configuration is not supported for CPC protocol
  UNUSED_VAR(bootloaderMode);
  UNUSED_VAR(baudTable);
  *serialPortFdReturn = NULL_FILE_DESCRIPTOR;
  snprintf(errorStringLocation, maxErrorLength, "Serial port setup not supported for CPC protocol mode\r\n");
  ...
  *serialPortFdReturn = NULL_FILE_DESCRIPTOR;
  ...
  while (true) { // break out of while on any error
    ...
    // Get serial port path - only needed for ASH/SPI modes
    const char* serialPortPath = (const char*)readConfig(serialPort);
    if (!serialPortPath) {
      snprintf(errorStringLocation, maxErrorLength, "Serial port path not configured\r\n");
      break;
    }

    *serialPortFdReturn = open(serialPortPath,
