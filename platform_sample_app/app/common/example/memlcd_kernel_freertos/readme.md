# MEMLCD Kernel FreeRTOS

This example demonstrates the Memory LCD (MEMLCD) module in a FreeRTOS kernel task using the Silicon Labs Graphics Library (glib).
...
This example uses the Memory LCD module and glib in a FreeRTOS task. Button 0 clears the LCD; Button 1 prints "Hello World!" on the LCD. Use it to learn MEMLCD and glib in a multitasking application.
...
- **No display output:** Verify MEMLCD connections and LCD type; check SPI/pin configuration and FreeRTOS task scheduling.
- **Buttons not responding:** Ensure button GPIOs match the board and are configured in the project.
- **Build errors:** Verify target part and MEMLCD, glib, and FreeRTOS components.
