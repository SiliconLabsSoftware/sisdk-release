                              ...
                              sl_zigbee_gpd_t_t * gpd)
{
  ...
  bool isIeee = (gpd->addr.appId == SL_ZIGBEE_GPD_APP_ID_IEEE_ID);
  ...
  // Extended FC is always present for IEEE AppId; for SrcId only when security or rxAfterTx
  if (isIeee
  ...
  // Direction from GPD to GPP is 0 ,so save a byte of code by not adding it! //B[7]
  ...
  // MAC frame control: IEEE AppId uses long source address mode
  if (isIeee) {
  ...
  // IEEE AppId: include source IEEE address in MAC header
  if (isIeee) {
  ...
  // SrcId AppId: include 4-byte source ID in NWK layer
  if (!isIeee) {
    sl_zigbee_gpd_utility_copy_4_bytes(&buffer[index], gpd->addr.id.srcId);
    index += 4;
  } else {
    // IEEE AppId: include 1-byte endpoint in NWK layer
                                 ...
                                 sl_zigbee_gpd_t_t * gpd)
{
  ...
  // NWK FC position depends on runtime AppId (IEEE adds 8-byte source address to MAC header)
  uint8_t posNwkHdr = (gpd->addr.appId == SL_ZIGBEE_GPD_APP_ID_IEEE_ID)
                      ? SL_ZIGBEE_GPD_MAC_HEADER_SIZE_IEEE_TX
                      : SL_ZIGBEE_GPD_MAC_HEADER_SIZE;
...
{
  bool isIeee = (sl_zigbee_gpd_get_gpd()->addr.appId == SL_ZIGBEE_GPD_APP_ID_IEEE_ID);
  uint8_t authStart  = isIeee ? SL_ZIGBEE_GPD_AUTH_START_IEEE     : SL_ZIGBEE_GPD_AUTH_START_SRC_ID;
  uint8_t authFixLen = isIeee ? SL_ZIGBEE_GPD_AUTH_FIXED_LEN_IEEE : SL_ZIGBEE_GPD_AUTH_FIXED_LEN_SRC_ID;
                                            ...
                                            uint16_t length)
{
  ...
  if (nwkFcExtnPresent == true
      && !(nwkFc & SL_ZIGBEE_GPD_NWK_FC_FRAME_TYPE_MASK)) { // Only for data frames
    addr.appId = appId;
    if (appId == SL_ZIGBEE_GPD_APP_ID_SRC_ID) {
      addr.id.srcId = *((uint32_t *)(&(buffer[currentFinger])));
      currentFinger += 4; // SrcId size is 4 bytes
    } else if (appId == SL_ZIGBEE_GPD_APP_ID_IEEE_ID) {
      // IEEE address comes from the MAC header long source address field
  ...
  if (!(nwkFc & SL_ZIGBEE_GPD_NWK_FC_FRAME_TYPE_MASK)) { // data frames only
    uint8_t authStart  = (appId == SL_ZIGBEE_GPD_APP_ID_IEEE_ID)
                         ? SL_ZIGBEE_GPD_AUTH_START_IEEE
                         : SL_ZIGBEE_GPD_AUTH_START_SRC_ID;
    uint8_t authFixLen = (appId == SL_ZIGBEE_GPD_APP_ID_IEEE_ID)
    ...
    if (securityLevel == SL_ZIGBEE_GPD_SECURITY_LEVEL_LONG_AUTH_ONLY) {
      sl_zigbee_decrypt_rx_buffer(buffer,
                                  authStart - 1,
                                  0,
                                  nwkPayloadLength - authStart + 1 - SL_ZIGBEE_GPD_MIC_LENGTH);
    } else if (securityLevel == SL_ZIGBEE_GPD_SECURITY_LEVEL_LONG_FULL) {
      sl_zigbee_decrypt_rx_buffer(buffer,
                                  authStart,
                                  nwkPayloadLength - authStart + 1 - authFixLen - SL_ZIGBEE_GPD_MIC_LENGTH,
                                  authFixLen);
    }
  }
