# External SPI Flash Storage Bootloader (single slot)

Demonstrates how to configure the Gecko Bootloader to store firmware update images using external SPI flash, in a single-slot setup with a 512 kB slot at 0x0.

This project provides a **storage-based** Gecko Bootloader for Silicon Labs Series 2 SoCs that use **external SPI flash** to hold upgrade images. The bootloader identifies the SPI flash chip from a curated set of supported parts (Macronix, Winbond, Micron, ISSI, Cypress / Spansion families) — for SFDP-based auto-detection use the `bootloader-storage-spiflash-sfdp-single` variant instead. Unlike the internal-storage variants, the bootloader does not consume any of the MCU's internal flash for upgrade staging — the entire main flash region is available to the application. The bootloader does not itself receive the upgrade image over a communication link; the running application downloads the GBL upgrade image (over any transport — BLE, Wi-SUN, OpenThread, Zigbee, UART, etc.) and writes it into the **single 512 kB storage slot** at offset `0x0` of the external SPI flash. On the next reboot the bootloader verifies the image in the slot and installs it over the application. The slot layout can be adjusted on the **Storage** tab of the project configurator.
...
Use this sample (instead of the SFDP variant `bootloader-storage-spiflash-sfdp-single`) when you need deterministic, vetted support for a specific known-good SPI flash chip on your board — the chip ID is checked against an explicit list and unsupported parts are rejected at boot. Use the dual-slot variant (`bootloader-storage-spiflash`) when you need A/B staging or fail-safe upgrade flows.

## Prerequisites / Setup Requirements

**Hardware**
- A Silicon Labs Series 2 SoC development kit with an **on-board external SPI flash chip** that is supported by the `bootloader_spiflash_storage` component (typically Macronix MX25-series, Winbond W25Q, Micron N25Q, ISSI IS25, or Cypress / Spansion S25 / S26 parts; check the kit user guide). The `hardware:component:memory:spi` tag restricts this sample to boards with a SPI flash present.
...
1. In Simplicity Studio, create a new project from **Bootloader - SoC SPI Flash Storage (single image)** for your target Series 2 SoC kit with on-board SPI flash.
...
- **Slot does not fit in SPI flash:** Verify the slot end (`SLOT0_START + SLOT0_SIZE`, default `0x80000`) is within the size of the SPI flash chip on the board. With the default 512 kB layout, the chip needs to be at least 512 kB.
