...
int32_t storage_readRaw(uint32_t address, uint8_t *data, size_t length)
{
  ...
  if (!spiflashVerifyAddressRange(address, length, NULL)) {
...
int32_t storage_writeRaw(uint32_t address, const uint8_t *data, size_t numBytes)
{
  ...
  // Ensure address is is within chip
  if (!spiflashVerifyAddressRange(address, numBytes, NULL)) {
    return BOOTLOADER_ERROR_STORAGE_INVALID_ADDRESS;
  }
...
int32_t storage_eraseRaw(uint32_t address, size_t totalLength)
{
  ...
  if (!spiflashVerifyAddressRange(address, totalLength, &deviceType)) {
    return BOOTLOADER_ERROR_STORAGE_INVALID_ADDRESS;
  }
