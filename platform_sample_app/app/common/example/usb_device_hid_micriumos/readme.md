# USB Device HID Micrium OS

This example demonstrates the USB stack HID class with the Micrium OS kernel. When connected to a PC, the device acts as a mouse and triggers continuous mouse movements.
...
This Micrium OS example uses the USB stack HID class to implement a USB mouse. Once the device is connected to a computer, it enumerates as an HID mouse and drives continuous mouse movements so you can see the cursor move. Use it to learn USB HID device implementation with Micrium OS.
...
- **Build errors:** Verify target part has USB device support and that the USB stack (HID) and Micrium OS are configured.
