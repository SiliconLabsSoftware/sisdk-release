...
__STATIC_INLINE void _led_mutex_acquire(void)
{
  EFM_ASSERT(osMutexAcquire(_led_mtx, osWaitForever) == osOK);
}

/* Mutex release */
__STATIC_INLINE void _led_mutex_release(void)
{
  EFM_ASSERT(osMutexRelease(_led_mtx) == osOK);
}
