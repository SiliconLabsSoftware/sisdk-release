# SPIDRV Slave Bare-metal (USART)

This example demonstrates the SPI driver in slave mode using USART in a bare-metal configuration. Connect two kits to exchange data; log output over VCOM.
...
This bare-metal example uses the Serial Peripheral Interface in slave mode (USART). Install this app on one kit and the SPI master example on a second kit; connect them via the expansion header (CS–CS, SCLK–SCLK, MOSI–MOSI, MISO–MISO). The two devices periodically exchange data; exchanges are logged over VCOM. Use it to learn SPIDRV slave operation with USART without an RTOS.
...
- **Build errors:** Verify target part has USART and that SPIDRV is configured for slave mode.
