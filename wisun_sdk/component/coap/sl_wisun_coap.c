...
/**************************************************************************//**
 * @brief wrapper function for sl_malloc
 * @details casts the input param from uint16_t to size_t type
 * @param[in] size size to allocate
 * @return  void* Pointer to allocated memory
 *****************************************************************************/
__STATIC_INLINE void *_sl_malloc_wrapper(uint16_t size);

/**************************************************************************//**
 * @brief wrapper function for sl_free
 * @details wrapper function to check heap memory before freeing
 * @param[in] addr pointer to memory to free
 *****************************************************************************/
static void _sl_free_wrapper(void *addr);
                        ...
                        const sl_wisun_coap_version_t   version)
{
  ...
  EFM_ASSERT(_wisun_coap_mtx != NULL);

  _wisun_coap_mutex_acquire();

  // init handler
  _coap.malloc      = &_sl_malloc_wrapper;                        // malloc
  _coap.free        = &_sl_free_wrapper;                          // free
...
char * sl_wisun_coap_get_uri_path_str(const sl_wisun_coap_packet_t * const packet)
{
  ...
  uri_path = (char *)sl_malloc((uint16_t)buff_len);
  if (uri_path == NULL) {
    return NULL;
  }
...
char * sl_wisun_coap_get_payload_str(const sl_wisun_coap_packet_t * const packet)
{
  ...
  str = (char *)sl_malloc(packet->payload_len + 1U);
  if (str == NULL) {
    return NULL;
  }
...
__STATIC_INLINE void _wisun_coap_mutex_acquire(void)
{
  if (osKernelGetState() == osKernelRunning) {
    EFM_ASSERT(osMutexAcquire(_wisun_coap_mtx, osWaitForever) == osOK);
  }
}
...
{
  if (osKernelGetState() == osKernelRunning) {
    EFM_ASSERT(osMutexRelease(_wisun_coap_mtx) == osOK);
  }
}
